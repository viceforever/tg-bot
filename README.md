...
- **DOWNLOAD_PATH** - (опционально) Путь для хранения скачанных файлов (по умолчанию: ./downloads)
...
- `/files <chat_id> <days>` - Получить файлы (фото, документы, аудио и т.д.) за последние N дней
  - Пример: `/files -5148403988 7` - получить файлы за последние 7 дней
...
Все файлы (фото, документы, видео, аудио, голосовые сообщения, стикеры) автоматически скачиваются на диск при получении сообщения. Файлы организованы в следующую структуру:

```
downloads/
├── chat_<chat_id>/
│   ├── 2025-01/
│   │   ├── photo_20250115_143022.jpg
│   │   ├── document_20250115_143025_report.pdf
│   │   └── voice_20250115_150000.ogg
│   └── 2025-02/
│       └── ...
```

В базе данных хранится локальный путь к файлу, что позволяет:
- Быстро находить и отправлять файлы администратору
- Показывать информативные пути вместо непонятных file_id
- Работать с файлами даже при отсутствии интернета
